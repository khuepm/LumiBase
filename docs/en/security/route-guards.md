...
| PR #130 (bug portion ported) | `/auth/register` was on the `withAuth` bypass list while its handler read the principal (guaranteed 500), and bound new users with the literal role id `'member'` (FK violation; the seeded member role is a nanoid). |
