...
Cái ghim đó có một hệ quả nên biết trước khi nó làm bạn bất ngờ: `init` lấy scaffolder từ **registry**, không phải từ repository này. Nên một template đã có trong source tree vẫn **không** dùng được qua `lumibase init` cho tới khi `create-lumibase` được publish ở đúng phiên bản của CLI — trước lúc đó lệnh fail *bên trong* scaffolder, với lỗi thiếu thư mục template chứ không phải lỗi template không tồn tại. Tên là hợp lệ; chỉ là artifact đã publish ra đời trước nó. `npm create lumibase@latest` cũng bị đúng ràng buộc này vì cùng lý do. Nếu bạn đang làm từ một checkout và cần một template chưa publish, hãy chạy scaffolder trực tiếp từ repo thay vì đi qua một trong hai đường vào.
