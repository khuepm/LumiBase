...
| B9 | 2026-07-25 · fix-issues-296 (sponsor rewards store) | bug (feature gap) | `apps/landing` rewards flow | Cả luồng claim reward **không chạy được trên production**: `apps/landing` build `output: 'export'` (static) nên 2 route handler (`api/rewards/claim`, `api/webhooks/github-sponsors`) nằm ở `src/_api-routes-disabled/` và không có trong bundle — trang `/rewards/claim` vẫn `fetch('/api/rewards/claim')` ⇒ 404 → user thấy "An error occurred". Issue #296 chỉ vá tầng lưu trữ (stub `saveToDatabase()` no-op + claim không atomic), KHÔNG bật lại route. Muốn dùng thật cần server runtime (vd `@cloudflare/next-on-pages`) + wire `configureSponsorStore(resolveSponsorStore(env))` + binding D1 `SPONSORS_DB` | medium | `open` | Store + migration + hướng dẫn wiring đã sẵn: `apps/landing/src/lib/rewards/`, `apps/landing/migrations/0001_sponsors.sql`, mục "Sponsor rewards store" trong `apps/landing/README.md`. Registry setup-impact #84 |
| B10 | 2026-07-25 · fix-issues-296 (sponsor rewards store) | vuln (minor) | `apps/landing/src/_api-routes-disabled/api/webhooks/github-sponsors/route.ts` | `crypto.timingSafeEqual(Buffer.from(signature), Buffer.from(expectedSignature))` **throw** khi 2 buffer khác độ dài (`signature` là header do client kiểm soát) ⇒ signature sai độ dài trả `500 Internal server error` thay vì `401`, và `WEBHOOK_SECRET` mặc định `''` nghĩa là thiếu env → mọi request vẫn được HMAC bằng secret rỗng. Không exploit được hiện tại vì route đang disabled (B9) | low | `open` | Vá khi bật lại route: check độ dài trước khi so sánh (hoặc so digest cùng độ dài) + fail-fast khi `GITHUB_WEBHOOK_SECRET` vắng |
