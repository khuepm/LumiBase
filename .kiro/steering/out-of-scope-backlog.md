...
| B8 | 2026-07-23 · owasp-api-top-10-audit | vuln (deps) | `apps/landing`, `apps/consumer`, transitive | Gate CI `Dependency vulnerability audit` đỏ trên PR #289 (23 advisory: 11 high/11 moderate/1 low) — KHÔNG do diff PR này (`git diff origin/main...HEAD` cho `package.json`/lockfile rỗng). Nổi bật: Next.js SSRF `GHSA-89xv-2m56-2m9x` (Server Actions) + `GHSA-p9j2-gv94-2wf4` (rewrites) ở `next@16.2.10` (landing/consumer, KHÔNG thuộc CMS API); `fast-uri@3.1.2` host-confusion `GHSA-4c8g-83qw-93j6` (transitive). Không nằm trong dependency-path của CMS API (Hono) | high | `open` | Cần PR riêng bump `next ≥16.2.11` + `fast-uri ≥3.1.3` (đổi lockfile — tách khỏi PR docs #289). FE guidance đã thêm ở `docs/{en,vi}/tutorials/nextjs-quickstart.md` §6. Xem `docs/en/security/owasp-api-top-10-audit.md` (API7/API8) |
