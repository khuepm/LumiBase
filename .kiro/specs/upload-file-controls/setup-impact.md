...
> **Single source of truth vẫn là `.kiro/specs/admin-setup-wizard/setup-impact.md`** (dòng #68).
...
- **Registry trung tâm**: đã ghi dòng #68 (settings key `upload_policy` + endpoint + guard hardening + polyglot scan). Cập nhật lại khi triển khai task F1 (re-encode) nếu nó thêm settings/capability.
