...
- **Phase A (crypto)**: KeyProvider dual-deploy + envelope/AAD + fail-closed + ItemService integration + migration + rotate endpoint — **xong**. Còn: rewrap worker (3.5), envelope mode (3.6), integration test DB (3.7).
- **Phase B (classification/audit)**: classification trong DSL/CompiledField/migration, guard 422, masking, field_access_log writer — **xong**. Còn: endpoint query access-log (5.3), integration test DB (5.4).
- **Phase C (scheduling)**: cột + validation `INVALID_PUBLISH_WINDOW` + delivery Publish_Window filter — **xong**. Còn: scheduler worker (7), Studio control (6.4).
- **Phase D (editorial)**: migration + state machine (transition table, gate, mapping) — **core xong**. Còn: endpoints + content_reviews wiring (8.3), Studio (8.5), integration test (8.6).
- **Phase E (erasure/retention/SAR)**: schema xong (migration). Còn: services + endpoints (9, 10).
- **Phase F (SEO)**: `_seo` builder + delivery emit + SDK helper + test — **xong**. Còn: ví dụ frontend (12).
