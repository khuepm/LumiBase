...
- [ ] 2. Zod schema chia sẻ
  ...
  - [ ] 2.3 Unit test schema: reject `HS256`/`none` trong `algorithms`, reject khoá lạ trong `claimMapping`, reject URL non-https ngoài dev, accept config tối thiểu hợp lệ (Req 2.3, 1.5; design §4) — **làm một phần**: HS256-config reject test ở DB-integration ✅; thiếu unit test riêng cho strict claimMapping/URL non-https
...
- [ ] 3. ExternalIssuerService
  ...
  - [x] 3.3 Audit `external_issuer_created/updated/deleted` với metadata `{ issuer, siteId }` — không token/claim (Req 2.7; design §5, §10) — **P1 (2026-07-07)**: `ExternalIssuerService.audit()` ghi 3 event, metadata `{issuer, issuerId}` + actor từ route; secret không log
...
- [ ] 7. Tích hợp vào `withAuth`
  ...
  - [x] 7.2 Giữ nguyên các path bỏ-qua-auth (`auth.ts:80-87`), dev-auth (`auth.ts:97-123`), nhánh CF Access (`auth.ts:125-159`); không đổi hành vi cũ (Req 7.6, 12.2; design §6.1) (CF Access bug hard-code roles:[admin] GIỮ NGUYÊN theo deviation — fix là v2)
  - [x] 7.3 Logging/error policy: dùng `formatSafeError`; không log raw token; response dùng mã chung `UNAUTHENTICATED`/`FORBIDDEN`/`TOKEN_EXPIRED`; audit `external_auth_denied` theo khuôn `auditApiKeyUseDenied` (`auth.ts:49-69`) (Req 10.1-10.3; design §10 T6, T8) — **P1 (2026-07-07)**: middleware ghi `external_auth_denied` metadata `{code,status}` (không token/claim/reason)
  - [x] 7.4 Giới hạn kích thước token + độ lớn claim (chống parsing DoS) (Req 11.7; design §10 T7) — **P1 (2026-07-07)**: `MAX_TOKEN_CHARS=8192` reject trước decode (TOKEN_TOO_LARGE) + `MAX_ROLE_CLAIMS=50` cap role list trước khi resolve; test DoS-guard trong verifier.test.ts

### Phase D — Tests (bảo mật là trọng tâm)

- [ ] 8. Verifier unit + integration tests
  - [x] 8.1 `verifier.test.ts`: sinh keypair RS256/ES256 (jose `generateKeyPair`); accept token hợp lệ; reject `alg:none`, `HS256`, sai `aud`, sai `iss`, hết `exp`, `nbf` tương lai, `kid` lạ (cây quyết định design §10) (Req 13.1; design §10, §12) — **P1**: thêm case `nbf` tương lai ✅; `kid` lạ vẫn thiếu
  - [x] 8.2 Multi-tenant test: token site A + `X-Lumi-Site:B` → reject `SITE_MISMATCH`; cùng `iss` hai site dùng config độc lập (Req 13.2; design §6.4, §10 T3) — **P1**: thêm case cùng `iss` hai request-site độc lập ✅
  - [x] 8.3 Role-mapping test: default-deny → 403 `NO_ROLE_MAPPING`; map→role `adminAccess=true` → principal admin qua `PermissionService`; xác nhận KHÔNG có path hard-code `['admin']` (Req 13.3; design §6.5, §8, §10 T2) (nhánh CF Access vẫn hard-code admin — ngoài scope external path, v2)
  - [x] 8.4 JIT test: on → tạo `users`+`userSites` idempotent (chạy 2 lần không trùng); off → 403 `USER_NOT_PROVISIONED`; user inactive → 401 (Req 13.4; design §6.6) — **P1**: thêm case user inactive → 401 ✅
  - [ ] 8.5 Chain test: API key ưu tiên hơn external; bearer non-JWT → fallback custom JWT; `iss` khớp + verify fail → KHÔNG fallback (fail-closed) (Req 13.5; design §6.1, §10 T9) — **chưa làm**: không có chain test (API key ưu tiên / fallback / fail-closed)
  - [ ] 8.6 Admin CRUD integration test: tạo/sửa/xoá issuer; enforce `adminAccess` (403 cho non-admin); reject `HS256` trong `algorithms`; reject thiếu JWKS source (Req 13.6, 2.2, 2.3; design §5) — **làm một phần**: CRUD + duplicate + HS256 ✅; thiếu test 403 non-admin + reject thiếu JWKS source
