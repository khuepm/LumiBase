...
- [x] 1. Filesystem migration & config
  - [x] 1.1 Mở rộng `docs.config.json` với `i18n.localeNames` và validate `defaultLocale ∈ locales`
    - Thêm `i18n.localeNames: { en: "English", vi: "Tiếng Việt" }`
    - Đảm bảo `defaultLocale: "en"` nằm trong `locales`
    - _Requirements: 7.1, 7.2, 7.5_

  - [x] 1.2 Migrate `docs/*` → `docs/en/*` bằng `git mv`
  ...
  - [x] 1.3 Update tất cả internal markdown links trong `docs/en/**/*.md` không cần thay đổi
...
- [x] 2. Multi-locale plugin (build-time registry)
  - [x] 2.1 Mở rộng `vite-plugin-docs-loader.ts` để discover theo locale
  ...
  - [x] 2.2 Build `MultiLocaleDocRegistry` structure
  ...
  - [x] 2.3 Update virtual module exports
  ...
  - [x] 2.4 Wire config vào plugin trong `vite.config.ts`
    - Import `docs.config.json` rồi pass `config` option vào plugin factory
    - _Requirements: 7.1_

  - [x] 2.5 Unit test cho plugin multi-locale
...
- [x] 3. URL routing với explicit locale prefix
  ...
  - [x] 3.4 Tạo `LocaleGuard` component
...
- [x] 4. Hook & helpers
  ...
  - [x] 4.2 Tạo helper `useCurrentSlug()`
    - Extract slug từ `useLocation().pathname` bằng `parseUrl()`
    - Trả về string (rỗng nếu không có)

  - [x] 4.3 Tạo `translations/ui.ts` + helper `t(key, locale, params?)`
  ...
  - [x] 4.4 Tạo hook `useT()` wrapping `t(...)` với locale từ `useLocale()`
    - Convenience để components không phải pass locale mỗi lần
    - _Requirements: 6.5_

  - [x] 4.5 Unit test useLocale
...
- [x] 5. Components
  ...
  - [x] 5.2 Mount LocaleSwitcher vào `Layout.tsx` header
    - Đặt cạnh SearchDialog, bên phải navbar
    - _Requirements: 3.1_

  - [x] 5.3 Update `Layout.tsx` dùng `useT()` cho navbar items label
  ...
  - [x] 5.4 Update `Layout.tsx` dùng `useT()` cho footer column titles + copyright + sidebar empty state
    - Áp dụng `resolveLabel()` cho footer column titles
    - _Requirements: 6.4, 6.5_

  - [x] 5.5 Update `Layout.tsx` extract `activeSlug` qua `useCurrentSlug()` và pass vào Sidebar
    - Replace logic hard-code `/docs/` prefix bằng `parseUrl()`
    - _Requirements: 2.1_

  - [x] 5.6 Update `Sidebar.tsx` mark slug missing ở locale active
...
- [x] 6. DocPage fallback + Translation Banner
  - [x] 6.1 Tạo `lib/resolveDoc.ts` với `resolveDoc(locale, slug)`
  ...
  - [x] 6.2 Property test 2 — Fallback chain correctness
  ...
  - [x] 6.3 Update `DocPage.tsx` dùng `useLocale()` + `resolveDoc()`
  ...
  - [x] 6.4 Tạo `TranslationBanner` component
...
- [x] 7. Internal link rewriting với locale awareness
  - [x] 7.1 Update `LinkRewriter` (`MarkdownRenderer`'s anchor override) nhận thêm `currentLocale`
  ...
  - [x] 7.2 Update `MarkdownRenderer` pass `currentLocale` xuống `LinkRewriter`
    - _Requirements: 8.1_

  - [x] 7.3 Update tests cho `LinkRewriter` với multi-locale
    - Verify href có locale prefix
    - Verify giữ locale khi navigate
    - _Requirements: 8.1, 8.2_

- [x] 8. Search per-locale
  - [x] 8.1 Refactor `lib/search.ts` thành `getSearchIndex(locale)` lazy cache
  ...
  - [x] 8.2 Update `SearchDialog.tsx` consume `useLocale()`
  ...
  - [x] 8.3 Property test 4 — Search locale isolation
    - **Property 4**: với mọi `(L, q)`, `search(L, q)` chỉ trả slug ∈ `docSlugsByLocale[L]`
    - **Validates: Requirements 5.1, 5.2, 5.3**

- [x] 9. NotFound + edge cases
  - [x] 9.1 Update `NotFoundPage` dùng `useT()` cho title và home link
