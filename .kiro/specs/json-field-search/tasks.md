...
- [x] 1. Path model & validation (pure, chốt bảo mật)
  ...
  - [x] 1.2 Thêm `FieldPath` interface + `resolveFieldRef(key)` (dot-split + structural check) + `flattenNested(obj)` (NestedObject → segments + leaf op-object); nhập nhằng op+nested cùng cấp → `INVALID_FILTER` (Req 1.1, 1.3, 2.1-2.3; design §3, §4)
...
- [x] 2. `fieldExpression` mở rộng cho nested path
  ...
  - [x] 2.2 Cập nhật `buildFilter()` (item-service.ts:230-301) gọi `resolveFieldRef`+`validatePath` cho mỗi leaf, xử lý NestedObject form, đếm leaf-clause cho `MAX_FILTER_CLAUSES`; giữ walk `_and`/`_or` (item-service.ts:234-246) (Req 2.4, 5.3, 6.4; design §3, §4)
  - [x] 2.3 Test parameter-binding: khẳng định `path.segments` nằm trong `sql.params`, KHÔNG trong queryChunks literal (không có nội suy chuỗi) (Req 6.2, 10.2; design §5, §9)

- [ ] 3. Type-aware Cast_Rule — **HOÃN v2 có chủ đích** (deviation 2026-06-22: v1 so sánh text để giữ additive + tránh cast-error sập query; xem Implementation status)
...
- [x] 5. Sort theo JSON path
  ...
  - [x] 5.2 Unit test sort: `-metadata.priority` numeric đúng thứ tự kiểu; structural sort không đổi; segment bẩn trong sort token bị reject (Req 9; design §5.4)
...
- [x] 7. Backward-compat & integration tests
  ...
  - [x] 7.2 Backward-compat test: filter top-level cũ (`{title:{_eq}}`) + Structural_Field (`status`, `created_at`) cho kết quả KHÔNG đổi; tenant scope giữ (kết quả không leak cross-site) (Req 7.1, 7.2, 7.5; design §10)
