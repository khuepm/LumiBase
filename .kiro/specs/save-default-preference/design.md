...
1. **Default value nên là `'return'` hay `'stay'`?** Complaint gốc (writer khó chịu vì *return* mặc định) gợi ý `'stay'` thân thiện hơn. Nhưng đổi default cứng sang `'stay'` sẽ **thay đổi hành vi cho mọi instance đang chạy** khi họ chưa đặt site default — rủi ro ngạc nhiên. Quyết định v1: **giữ `'return'` làm `Hardcoded_Fallback` + `sites.default_save_action` DEFAULT `'return'`** để zero behavior change; admin/user tự đổi sang `'stay'`. (Lưu ý: hành vi *code* hiện tại của `item-detail.tsx` thực ra là "stay" — save không navigate; nên chọn fallback `'return'` sẽ là một thay đổi nhỏ có chủ đích để khớp kỳ vọng Directus "return to list". Cần xác nhận với maintainer xem có muốn fallback `'stay'` để khớp hành vi code hiện tại thay vì hành vi Directus.) — *Chốt khi review: ưu tiên không đổi hành vi cảm nhận của writer hiện tại.*
