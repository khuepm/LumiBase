...
6. **Instance đã setup từ trước có cần backfill?** — KHÔNG. Migration `0038_git_integration` chỉ `CREATE TABLE IF NOT EXISTS` (additive, idempotent) + thêm bảng vào `rls-policies.sql`; instance cũ nhận bảng rỗng → no-op. Role/grant seed lazy/on-connect nên không cần backfill.
