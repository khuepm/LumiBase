...
| `setupToken` | sinh lúc startup khi `state=uninitialized && LUMIBASE_REQUIRE_SETUP_TOKEN=true`, in stdout đúng một lần, lưu hash sha256 vào `setup_token_hash` | Compare bằng constant-time hash compare. Hết hạn ngay khi state→initialized. **Chỉ Node/Docker** (#470): `serve.ts` await `runSetupTokenStartup` (`modules/setup/startup.ts`) trong mọi tiến trình HTTP, trước `serve()`; ghi có điều kiện nên nhiều replica cùng boot chỉ in một token. Restart **không** in lại (hash còn thì giữ) — log chỉ cách xoá hash để sinh token mới. Cloudflare Workers không có startup ⇒ cờ không hỗ trợ; `/setup/complete` trả `503 SETUP_TOKEN_NOT_ISSUED` kèm hướng dẫn thay vì `SETUP_TOKEN_REQUIRED` câm. |
