...
- **Phase P0 (tasks 1–6): đã triển khai + rà DoD 2026-07-05.** typecheck workspace sạch; test CMS pass. Setup Impact Registry #37 = `n/a`. CHANGELOG [Unreleased] cập nhật. Task 1.3 (edge-cache adapter) landed sau P0 closeout.
- **Bổ sung Req 19 / tasks 22.x (cache penetration): follow-up DoD 2026-08-01.** k6 `load-penetration.js`: **DB-query-per-404 = 0.0308** (≤ 0.05) → roadmap §2; Grafana negative hits/writes; §21.6 giữ `LUMIBASE_DELIVER_RATE_LIMIT=1200`.
