...
- [ ] 22. Cache penetration defence
  ...
  - [x] 22.6 **[P1, cần 22.4]** `forget` sau commit tại write path đã có API (tạo collection, tạo item, tạo site) — cùng vị trí và cùng chính sách lỗi với tag purge task 9.1 (lỗi → metric + warn, không fail request) (Req 19.7; design §14.7). Test P19: tạo tài nguyên đang bị tombstone → 200 ngay, không chờ TTL.
    ...
    - **Bỏ — item tombstone (rà code 2026-08-02):** `forget` cho `neg:*:item:*` đã bị xoá khỏi `ItemService.create`. Không đường nào ghi khoá đó (Req 19.8 cấm serve tombstone cho request có credentials, mà mọi đọc item-by-id đều sau auth), nên call-site chỉ tốn một round-trip Redis xoá thứ không tồn tại. Xem design §14.5.
