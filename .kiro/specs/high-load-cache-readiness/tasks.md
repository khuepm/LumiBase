...
- [x] 2. Permission cache invalidation (key versioning)
  ...
  - [x] 2.3 Integration test: `permission-cache-revocation.integration.test.ts` — MemoryCacheProvider + API-key principal, bumpVersion → recompile (not stale). Auth-layer revoke → 401: `api-key-security.test.ts`. **Bổ sung sau:** Postgres E2E "revoke → 403 content" đã có ở `routes/__tests__/permission-revocation.db.integration.test.ts` — policy/permission/api_key_policies thật, assert HTTP 403 chứ không phải giá trị trả về của service, và assert entry v1 vẫn còn trong cache để chứng minh 403 đến từ bump chứ không từ TTL. Kèm ca ngược: quên bump → vẫn 200 (ghim blast radius của route đổi quyền mà không gọi `bumpPermissionVersion`). Ghi chú loại trừ cũ ("cần DB harness; behavioural bump test đủ cho cache path") không còn đúng (Req 2.5; design §13.1)
...
- [x] 7. Chốt phase P0
  - [ ] 7.1 Re-run k6 suite, điền bảng roadmap §2 cột "Sau P0" (Req 0.3) — code của P0 đã land; **phép đo chưa chạy**. Baseline đã có (task 0, PR #360) nên chỉ cần `apps/cms/k6/baseline/run.sh` với `SKIP_SEED=true`
...
- [x] 15. Chốt phase P1
  - [ ] 15.1 Re-run k6; kiểm origin offload delivery ≥90% trên `load-deliver.js`; điền roadmap §2 — code của P1 đã land; **offload chưa đo**. Chỉ dòng DB-query-per-404 (0.0308) là số thật
...
- [x] 21. Chốt chương trình
  - [ ] 21.1 Re-run toàn bộ k6, hoàn tất bảng roadmap §2; so sánh với target — code toàn chương trình đã land; **chưa re-measure**. Chênh lệch so với target phải ghi nhận công khai, không sửa số (Req 0.3)
