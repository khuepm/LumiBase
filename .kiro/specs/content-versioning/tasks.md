...
- [x] 1. Shared diff type + helper
  - [x] 1.1 Đưa `Change` interface vào `packages/shared/src/` (nguồn truth FE/BE/SDK); viết `diffFields(before, after): Change[]` tái hiện đúng logic shallow của `revisions-diff.tsx` — done: `packages/shared/src/schemas/diff.ts`
...
- [x] 2. Schema + migration
  ...
  - [x] 2.2 Migration viết tay + sửa journal ([[migrations-are-hand-written]]); `pnpm -F @lumibase/database db:migrate` chạy sạch (migration đã gộp vào `0000_lumibase_init` sau table-prefix refactor)
    - _Requirements: 1.3_
...
- [ ] 5. SDK
  - [ ] 5.1 Thêm `ContentVersion` + `VersionCompare` types và 7 method vào SDK items namespace (backward-compatible) — **chưa làm**: SDK không có `ContentVersion`/methods — Studio gọi qua fetch nội bộ trong `version-panel.tsx`
    - _Requirements: 2.x, 5.1_

- [ ] 6. UI editor
  - [ ] 6.1 `version-switcher.tsx`: dropdown Main + versions (useQuery), New version dialog (auto-slug key), Delete; gọi SDK — **khác thiết kế**: giao hàng dạng `version-panel.tsx` (list/create/delete trong panel), KHÔNG có dropdown switcher
    - _Requirements: 4.1, 4.2_
  - [ ] 6.2 `item-detail.tsx`: tích hợp switcher ở header; state `activeVersionKey`; khi != null editor đọc/ghi version.data (lưu → updateVersion, KHÔNG update item); `version-banner.tsx` hiển thị — **chưa làm**: không có `activeVersionKey` — editor không đọc/ghi version.data; không có `version-banner.tsx`
    - _Requirements: 4.1, 4.4_
  - [x] 6.3 `version-compare-panel.tsx`: "Compare with main" → SDK.compareVersion → `<RevisionsDiff changes/>` — done trong `version-panel.tsx` (compare tái dùng RevisionsDiff), không file riêng
    - _Requirements: 4.3, 3.1_
  - [x] 6.4 Promote: nút + dialog xác nhận (cảnh báo nếu mainChanged); on success invalidate `['item']`,`['revisions']`,`['versions']`, reset activeVersionKey — done: promote mutation + invalidate; dialog cảnh báo mainChanged chưa verify
    - _Requirements: 4.3, 4.5, 5.2_
  - [ ] 6.5 Component test: switch Main↔version; banner; compare render diff; promote invalidate 3 query + cảnh báo mainChanged — **chưa làm**: không có component test
