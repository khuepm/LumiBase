...
- [x] 3. ReleaseService CRUD
  - [x] 3.1 Tạo `apps/cms/src/services/release-service.ts` class `ReleaseService` nhận deps `{ db, siteId, queue?, itemService }` (đối xứng `ItemService`); mọi select dùng `scopeSite(...)` (Req 11.1, 11.2; design §4)
  - [x] 3.2 `create(input)`: validate name (422), validate publishAt không quá khứ, insert với `status = publishAt ? 'scheduled' : 'draft'`, `createdBy` (Req 1.1-1.5, 6.2; design §4.1)
  - [x] 3.3 `list(filter)` + `get(id)`: list scoped + filter status + phân trang `{ data, meta }`; get join release_items, 404 nếu thiếu (Req 4.1-4.5; design §4.3)
  - [x] 3.4 `delete(id)`: xoá scoped, release_items cascade, cho phép mọi trạng thái, trả 204 (Req 9.1-9.5; design §4.7)

- [x] 4. Add/remove items + revision pin
  - [x] 4.1 `patch(id, {...})` addItems/removeItems: verify item tồn tại trong site (ITEM_NOT_FOUND), validate targetStatus, upsert theo unique key, chặn sửa khi `published` (RELEASE_IMMUTABLE 409) (Req 2.1-2.7; design §4.2)
...
- [x] 7. Release_Sweep
  - [x] 7.1 Thêm `runReleaseSweep(deps)` vào (hoặc sibling của) `apps/cms/src/services/scheduler-worker.ts`; đăng ký trên cùng queue `'content-scheduler'` cạnh `runSchedulerTick` (`scheduler-worker.ts:265-269`) qua `QueueProvider` injected (Req 6.4, 11.4; design §6)
  - [x] 7.2 Sweep query: per site_id `status='scheduled' AND publish_at<=now()` dùng `releases_publish_due_idx`, batch-bounded; gọi `ReleaseService.publish(id, {trigger:'scheduled'})` cùng đường code manual (Req 6.4, 6.5, 10.4; design §6)
  - [x] 7.3 Idempotent + maintenance window: conditional guard `status!='published'`; nếu `maintenanceWindow` set và now ngoài cửa sổ → hoãn tới tick sau (Req 6.6, 6.7; design §6)
...
- [x] 8. Audit & provenance
  ...
  - [x] 8.2 Test audit: assert event + metadata đúng cho manual success, scheduled success, partial, failed (Req 12.1-12.3; design §7)

### Phase E — Docs, Setup Impact, DoD

- [x] 9. Docs & API spec
  - [x] 9.1 Cập nhật `docs/en/api/hono-api-spec.md`: thêm `POST/GET /api/v1/releases`, `GET/PATCH/DELETE /api/v1/releases/:id`, `POST /api/v1/releases/:id/publish` (params, response, error codes ALREADY_PUBLISHED/EMPTY_RELEASE/RELEASE_IMMUTABLE/EDITORIAL_GATE_REQUIRED/PUBLISH_AT_IN_PAST) (DoD §4)
  - [x] 9.2 Cập nhật `docs/en/data-model.md`: thêm bảng `releases` + `release_items` (cột, FK, index, quan hệ tới items/revisions) (DoD §4)
  - [x] 9.3 CHANGELOG entry cho Content Releases + cập nhật **README Release policy** mô tả manual vs scheduled publish và ngữ nghĩa atomicity (DoD §4)
  - [x] 9.4 Trang docs ngắn mô tả workflow: tạo Release → add items xuyên-collection + pin revision → schedule/publish → outcome per item; nêu open questions (rollback v1 chưa hỗ trợ, late-binding race) (DoD §4; design §11)
