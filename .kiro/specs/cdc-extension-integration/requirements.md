...
- **Bảng `webhooks`** (`packages/database/src/schema/platform.ts:141`) — đã có `url/actions/collections/headers/secret/status`, tái dùng làm delivery target; spec này bổ sung dispatcher còn thiếu.
...
- **Change_Event**: Một bản ghi bất biến mô tả một mutation (create/update/delete) trên một item. Bảng `cdc_change_events` (transactional outbox, append-only).
...
- **Subscription**: Bản ghi đăng ký một consumer (pull / webhook / extension) kèm filter và checkpoint cursor. Bảng `cdc_subscriptions`.
...
- **Delivery**: Một lần thử phân phối một batch event tới một Subscription, có kết quả và attempt. Bảng `cdc_deliveries` (append-only).
...
1. THE Change_Feed SHALL cung cấp bảng `cdc_subscriptions` với các cột: `id` (nanoid, PK), `siteId` (FK, NOT NULL), `name` (text, NOT NULL — unique per site), `kind` (text, NOT NULL, `'pull' | 'webhook' | 'extension'`), `collections` (jsonb — filter, rỗng = tất cả), `operations` (jsonb — filter, rỗng = tất cả), `payloadMode` (text, NOT NULL, `'reference' | 'snapshot'`, default `'reference'`), `cursor` (uuid, nullable — checkpoint cuối đã ack/deliver thành công), `status` (text, NOT NULL, `'active' | 'paused' | 'dead' | 'stale'`, default `'active'`), `webhookId` (text, nullable, FK → `webhooks`), `extensionName` (text, nullable), `consecutiveFailures` (integer, default 0), `lastDeliveredAt` (timestamptz, nullable), `createdAt`, `updatedAt`.
...
1. THE Change_Feed SHALL scope mọi bảng mới (`cdc_change_events`, `cdc_subscriptions`, `cdc_deliveries`) theo `siteId`, thêm vào `rls-policies.sql`, và mọi query đều `where(eq(table.siteId, siteId))`.
...
| 6 | Backfill cho instance cũ? | **Không** | Migration chỉ `CREATE TABLE IF NOT EXISTS` (idempotent), 3 bảng mới rỗng; không đổi bảng cũ. |
