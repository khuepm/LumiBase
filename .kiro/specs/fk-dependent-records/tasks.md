...
- [ ] 1. DependentsService — reverse resolver
  ...
  - [ ] 1.3 Áp `permissions.whereFor()` cho reverse query (row-level) + `scopeSite` mọi select; thêm helper `isBlocking(groups)` = có group `restrict` với `count>0` (Req 1.6, 2.2, 9.2; design §3, §8) — **chưa làm (SECURITY)**: `scopeSite` ✅ mọi query, `isBlocking` ✅, nhưng KHÔNG áp `permissions.whereFor()` — preflight có thể lộ count/sample rows ngoài quyền đọc
  - [ ] 1.4 Unit test resolver: m2o/o2m/m2m, count vs sample limit, site isolation, group rỗng → [] (Req 1.2-1.7; design §4.1) — **làm một phần**: 7 DB-test (m2o count/onDelete, set-null-no-block…) ✅; thiếu m2m + site-isolation test

- [ ] 2. Preflight endpoint
  - [ ] 2.1 Thêm route `GET /items/:collection/:id/dependents` trong `apps/cms/src/routes/items.ts`: check `read` trên collection, 404 nếu target không tồn tại/soft-deleted, trả `{ data: { dependents, blocking } }` (Req 2.1-2.5; design §4.2, §5) — **làm một phần**: route ✅ trả report; thiếu check quyền `read` + 404 khi target không tồn tại
...
- [ ] 4. applyResolution core
  ...
  - [x] 4.2 Action `set_null`: set `data[manyField]=null` mọi dependent; check `manyField` required trong `fields` → abort `FIELD_REQUIRED` 409; cần quyền `update` trên `manyCollection` (Req 5.1-5.6, 9.1; design §4.3, §4.6) (quyền `update`: chưa gate — xem 5.2)
  - [x] 4.3 Action `reassign`: validate `newTargetId` tồn tại trong `oneCollection` & ≠ `id` → `INVALID_TARGET` 422; set `data[manyField]=newTargetId`; quyền `update` (Req 7.1-7.6, 9.1; design §4.3, §4.6) (quyền `update`: chưa gate — xem 5.2)
  - [x] 4.4 Action `delete`: delegate `ItemService.softDelete` (mặc định) / `hardDelete` khi `?hard=true` với `db=tx`; quyền `delete`; deindex/realtime chạy sau commit (Req 6.1-6.6, 9.1; design §4.3, §9.2) (quyền `delete`: chưa gate — xem 5.2)
...
- [ ] 5. Resolve endpoint
  ...
  - [ ] 5.2 Permission gating per action trên `manyCollection`; 403 `FORBIDDEN` khi thiếu; rollback nguyên tử khi bất kỳ bước fail (Req 5.5, 6.4, 7.4, 9.1-9.4; design §8) — **chưa làm (SECURITY)**: endpoint không gate quyền per action trên `manyCollection`; ItemService nội bộ dựng không `permissionCtx` → bypass check; set_null/reassign update SQL trực tiếp
...
- [x] 7. Docs
  - [x] 7.1 Cập nhật `docs/en/api/hono-api-spec.md`: thêm `GET /api/v1/items/:collection/:id/dependents` và `POST …/resolve-dependents` (params, body, response, error codes `DEPENDENT_RECORDS_EXIST`/`FIELD_REQUIRED`/`INVALID_TARGET`/`FORBIDDEN`/`NOT_FOUND`); ghi rõ ngữ nghĩa **soft-delete vs restrict** (chỉ restrict chặn; set null/cascade không tự chạy khi soft-delete) (Req 4.3, 4.5; DoD docs)
  - [x] 7.2 CHANGELOG entry cho feature; README/Release policy bump nếu version tăng — giữ narrative 0.5.0 (DoD docs)
