...
Current release: `v0.17.0` (`2026-07-03`) — **`lumibase_` table namespace & content releases**. Every system table is physically renamed to **`lumibase_<name>`** and the whole migration history is squashed into a single `0000_lumibase_init` (**breaking, fresh-install only** — a pre-prefix database must be dropped and recreated; the migrate runner refuses to run on a database carrying the pre-squash history, and collection names starting with `lumibase_`/`mat_` are rejected at the API). Riding on the new schema, this release also lands **Content Releases** (bundle item revisions across collections, publish manually or scheduled), **external JWT authentication** (trust an external IdP's JWKS with default-deny role mapping), **FK dependent-records handling** (structured 409 + resolve endpoints instead of orphaned references), **search inside JSON fields** (dotted paths + JSONB containment operators on the GIN index), and a **configurable default save action** (per-user preference over a site-wide default). It builds on **`v0.16.0`** (code-first configuration & auto-deploy from Flows), **`v0.15.0`** (realtime audience plane & cosmic design system), **`v0.14.0`** (push notifications & MCP path-traversal hardening), **`v0.13.0`** (deployment integrations & cross-collection search), **`v0.12.0`** (privacy & compliance suite, Directus-style interfaces & tenant isolation hardening), **`v0.11.0`** (Insights, content versioning & tenant-scoped search), **`v0.10.0`** (MCP everywhere), **`v0.9.0`** (regulated/sensitive content readiness), and the **`v0.5.0` Content OS foundation** — which remain the baseline this release builds upon, not replaces.

```bash
LUMIBASE_VERSION=0.17.0 docker compose -f docker/docker-compose.yml -f docker/docker-compose.prod.yml up -d
```
