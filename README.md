...
Current release: `v0.16.0` (`2026-07-03`) — **Code-first configuration & auto-deploy from Flows**. This release adds **Code-First Configuration**: export / diff / apply a site's schema configuration — collections, fields, relations, settings and webhooks — as a single declarative JSON manifest (`lumibase.config@v1`) via new admin-only `GET /api/v1/config/export` / `POST /api/v1/config/import` endpoints and a reworked `config export|diff|apply` CLI, for CI/CD and environment sync. Flows gain **`deploy:trigger` / `deploy:status`** operations, completing the auto-deploy-on-content-change path of deployment integrations. Security is hardened across the board: a new `withSiteMembership` middleware enforces tenant membership for user principals, dynamic extension dispatch is admin-gated again, `POST /auth/register` is fixed and fail-closed, `/api/v1/flows` joins the control-plane backstop, and a source-level tripwire suite locks the guard-chain wiring; the `lumibase_` collection name prefix is now reserved for platform-owned tables. **No schema migrations** in this release. It builds on **`v0.15.0`** (realtime audience plane & cosmic design system), **`v0.14.0`** (push notifications & MCP path-traversal hardening), **`v0.13.0`** (deployment integrations & cross-collection search), **`v0.12.0`** (privacy & compliance suite, Directus-style interfaces & tenant isolation hardening), **`v0.11.0`** (Insights, content versioning & tenant-scoped search), **`v0.10.0`** (MCP everywhere), **`v0.9.0`** (regulated/sensitive content readiness), and the **`v0.5.0` Content OS foundation** — which remain the baseline this release builds upon, not replaces.

```bash
LUMIBASE_VERSION=0.16.0 docker compose -f docker/docker-compose.yml -f docker/docker-compose.prod.yml up -d
```
