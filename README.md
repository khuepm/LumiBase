...
Current release: `v0.19.0` (`2026-07-07`) — **CWE Top 100 closeout, Visual Flow Builder triggers & marketplace community features**. The remaining 12 CWEs from the security audit are now mitigated (78/78 applicable weaknesses): no more wildcard-with-credentials CORS, a fail-closed CDC encryption key, atomic AI-approval decisions, a uniform 12-character password policy, revocable JWTs (`users.token_version`), real Cloudflare Access role resolution, a general API rate limiter, and redacted audit metadata. The **Visual Flow Builder** gains event/schedule/webhook triggers end to end, with the Studio editor now persisting the same canonical graph the runtime executes. The **marketplace** adds verified/trusted badges, download tracking, upvoting, and community submission + moderation. It builds on **`v0.18.0`** (custom domains & translation memory), **`v0.17.0`** (`lumibase_` table namespace & Content Releases), **`v0.16.0`** (code-first configuration & auto-deploy from Flows), **`v0.15.0`** (realtime audience plane & cosmic design system), **`v0.14.0`** (push notifications & MCP path-traversal hardening), **`v0.13.0`** (deployment integrations & cross-collection search), **`v0.12.0`** (privacy & compliance suite, Directus-style interfaces & tenant isolation hardening), **`v0.11.0`** (Insights, content versioning & tenant-scoped search), **`v0.10.0`** (MCP everywhere), **`v0.9.0`** (regulated/sensitive content readiness), and the **`v0.5.0` Content OS foundation** — which remain the baseline this release builds upon, not replaces.

```bash
LUMIBASE_VERSION=0.19.0 docker compose -f docker/docker-compose.yml -f docker/docker-compose.prod.yml up -d
```
