...
**Date:** 2026-07-11 · **Run:** automated (scheduled task, 18:04 +07) · **Coverage:** 10 headless CMS projects
**Previous run:** 2026-07-11 16:04 (+07)

> Sources this run: each project's **GitHub Issues** page (open-issue counts + newest open issues with ID/title/labels/open-date, quoted exactly as shown), fetched live and verified this cycle for all 10. Vendor **blogs were not fetched this run** — blog pages are client-rendered and were not re-hydrated this cycle, so all "latest blog post" lines are carried forward from prior runs and labeled `[Unverified]`. Version numbers not printed on a fetched page are `[Unverified]`. Security reads beyond issues explicitly labeled/titled as security are `[Inference]` (based on title/topic, not a confirmed advisory/CVE).

---

## TL;DR — Changes since previous run (2026-07-11 16:04)

**No change this cycle.** All 10 GitHub issue trackers returned the identical newest open issue (same ID, title, open date) and identical open-issue counts as the 16:04 run. Verified live this run.

**Blogs:** not re-fetched this cycle; latest-post lines carried forward as `[Unverified]`.
...
- **#27119** — Unable to register API extension hook: `document is not defined` — Bug / Ext SDK / Extensions / Low Impact — Apr 15, 2026
- **#27111** — Apple OAuth first_name / last_name not populated on registration — Apr 14, 2026
- **#27094** — `@directus/api` using old `tsdown` and `openid-client` — (auth dependency freshness) `[Inference: security-adjacent]` — Apr 11, 2026
...
- **#16214** — [MCP Plugin] create/update tools produce `{ type: 'null' }` for nullable relationship fields — plugin: mcp `[Inference: security-adjacent]` — Apr 8, 2026
...
- **#12812** — Feature request: preserve original image metadata / add IPTC metadata on upload — Feature / CLDX — May 10, 2026
- **#12787** — Feature request: support multiple `typegen` configurations — Feature / CLI — May 5, 2026
- **#12794** — Presentation tool writes `sanity.previewUrlSecret` to wrong dataset in multi-workspace hosted Studio — SAPP `[Inference: security-adjacent]` — Apr 16, 2026
...
- **#27445** — Security: add optional malware scanning for uploaded files (pompelmi) — feature request / needs:triage — Apr 17, 2026
...
- **#9789** — Should Keystone enforce GraphQL query depth limits by default? — Feature `[Inference: security-adjacent]` — Mar 18, 2026
...
- **#9665** — Field can be editable when `graphql.omit.update` is set — Bug / help wanted `[Inference: security-adjacent]` — Jul 22, 2025
...
- **#7148** — Folder-based collection with `create:false + delete:false` unnavigable — single-document auto-open fires inside folder views — bug / pending triage — Jul 4, 2026
...
**Newest / notable open issues (verified this run):**
- **#7875** — **Path traversal in decap-server proxy allows read/write/delete of files outside configured repository root** — type: bug **`[Inference: critical security]`** — Jul 5, 2026
...
**Newest / notable open issues (verified this run):**
- **#4501** — **Security: Cross-origin code execution via unvalidated postMessage in builder-block (CWE-346)** — **`[Inference: critical security]`** — Apr 4, 2026
...
- **#4212** — Using `eval` for detecting server code throws CSP error `[Inference: security-adjacent]` — Dec 15, 2025
...
- **#15398** — `@medusajs/icons` peer dep React ^19.2.5 conflicts with `@medusajs/dashboard`/`@medusajs/ui` (React ^18.3.1) — ERESOLVE — bug / requires-team — May 13, 2026
- **#15360** — [Bug] Race condition in cart promotions can create duplicate line-item adjustments — bug / requires-team / v2.0 `[Inference: security-adjacent]` — May 11, 2026
- **#15343** — `getDatabaseURL` in `@medusajs/test-utils` breaks for passwords with special URL characters (#, @, :) — bug / v2.0 — May 8, 2026
- **#15306** — Refund workflow can report success after partial refund failures (silent failure in `refundPaymentsStep`) — bug / needs triaging `[Inference: security-adjacent]` — May 6, 2026

**Blog:** `[Unverified]` — carried forward (*"Layout Composer in Medusa Admin"*, Jul 1). Not re-fetched this run.

---

## Method & caveats
- GitHub Issues pages fetched live this run (server-rendered; open counts and newest-issue rows quoted verbatim). All 10 verified.
- Vendor blogs are client-rendered and were not re-hydrated this cycle; blog lines are carried forward and labeled `[Unverified]`.
- `[Inference]` marks a security read derived from an issue's title/labels, not a confirmed CVE/advisory.
- Autonomous scheduled run; no user input. Next cycle should attempt a blog re-fetch to refresh `[Unverified]` post lines.

_Generated 2026-07-11 18:04 (+07)._
