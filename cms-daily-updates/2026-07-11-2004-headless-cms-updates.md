...
**Date:** 2026-07-11 · **Run:** automated (scheduled task, 20:04 +07) · **Coverage:** 10 headless CMS projects
**Previous run:** 2026-07-11 18:04 (+07)
...
## TL;DR — Changes since previous run (2026-07-11 18:04)

**No change this cycle.** All 10 GitHub issue trackers returned the identical newest open issue (same ID, title, open date) and identical open-issue counts as the 18:04 run. Verified live this run.
...
- **#27119** — Unable to register API extension hook because `document is not defined` — Bug / Ext SDK / Extensions / Low Impact / Med Reach — Apr 15, 2026
- **#27111** — Apple OAuth first_name/last_name not populated on registration — Apr 14, 2026
- **#27094** — `@directus/api` uses an old version of tsdown and openid-client — Apr 11, 2026 `[Inference]` security-adjacent (outdated auth dep)
...
**Blog:** `[Unverified]` — carried forward: *"An early look at Payload 4.0"* (Jun 9). Not re-fetched this run.
...
- **#12794** — Presentation tool writes `sanity.previewUrlSecret` to wrong dataset in multi-workspace hosted Studio — SAPP — Apr 16, 2026 `[Inference]` secret-scoping risk
...
- **#27445** — Security: add optional malware scanning for uploaded files (pompelmi) — needs:triage — Apr 17, 2026 (security feature request)
- **#27415** — Share button doesn't work because `portal.min.js` isn't loaded when subscriptions disabled — needs:triage — Apr 15, 2026

**Pinned:** 6.0 breaking-changes tracker (#23924), i18n mega-issue (#23361), renovate dependency dashboard (#13265).
...
- **#9665** — Field editable when `graphql.omit.update` is set — Bug / help wanted — Jul 22, 2025 `[Inference]` access-control gap
...
- **#7109** — Starter template 'tina-astro-starter' failed during build with npm using Node 24 — bug (CI-generated) — Jun 28, 2026
...
**Newest / notable open issues (verified this run):**
- **#7875** — **Path traversal in decap-server proxy allows read/write/delete of files outside the configured repository root** — type: bug — Jul 5, 2026 — **critical, trust-boundary vuln (persists)**
...
- **#7869 / #7868** — TypeError: Cannot destructure property 'url' of 'e.element.data' (duplicate reports) — Jun 28, 2026
...
**Newest / notable open issues (verified this run):**
- **#4501** — **Security: Cross-origin code execution via unvalidated postMessage in builder-block (CWE-346)** — Apr 4, 2026 — **still open (persists)**
...
- **#15398** — `@medusajs/icons` peer dep React ^19.2.5 conflicts with dashboard/ui (React ^18.3.1) — ERESOLVE in npm workspaces — bug / requires-team — May 13, 2026
- **#15371** — RTL layout issues in admin dashboard for Hebrew/Arabic/Farsi — bug / help-wanted — May 11, 2026
- **#15360** — [Bug] Race condition in cart promotions can create duplicate line-item adjustments — bug / requires-team / v2.0 — May 11, 2026 `[Inference]` data-integrity
- **#15306** — Refund workflow can report success after partial refund failures (silent failure in `refundPaymentsStep`) — bug — May 6, 2026 `[Inference]` payment-integrity
...
## Methodology & caveats
- **Issue data:** fetched live this run from each repo's public GitHub Issues page (`is:issue state:open`, default sort). Counts and newest-issue IDs/titles/dates quoted as displayed. Verified for all 10 repos.
- **Blogs:** not fetched this cycle (client-rendered; not re-hydrated). Latest-post lines are carried forward from earlier runs and labeled `[Unverified]`.
- **Security labels:** `[Inference]` tags mark items assessed as security-relevant from title/labels only, not confirmed CVEs/advisories. Two items carry explicit security signals from their own title/labels: Strapi #26494 (security label, critical), Decap #7875 (path traversal), Builder.io #4501 (CWE-346).
- **Version numbers** not printed on the fetched page are `[Unverified]`.
