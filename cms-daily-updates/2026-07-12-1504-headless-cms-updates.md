...
**Date:** 2026-07-12 · **Run:** automated (scheduled task, 15:04 +07) · **Coverage:** 10 headless CMS projects
**Previous run:** 2026-07-12 13:04 (+07)
...
## TL;DR — Changes since previous run (2026-07-12 13:04)

**GitHub issues: no change.** All 10 trackers returned the identical newest open issue (same ID, title, open date) and identical open-issue counts as the 13:04 run. Verified live this run. The last new issue observed across all trackers remains **TinaCMS #7169** (Jul 7).
...
- **#16214** — [MCP Plugin] create/update tools produce `{ type: 'null' }` for nullable relationship fields — plugin: mcp — Apr 8, 2026 `[Inference: MCP tooling correctness]`
...
- **#12733** — Unable to create new account — server rejects strong password as "too weak" — identity / Bug — Apr 22, 2026
- **#12794** — Presentation tool writes `sanity.previewUrlSecret` to wrong dataset in multi-workspace hosted Studio — SAPP — Apr 16, 2026 `[Inference: data-boundary/secret-scoping]`
...
- **#12568** — Dependency Dashboard — 🤖 bot — Apr 2, 2026
...
## Ghost
**Open issues:** 63 (verified). Stars ~52.8k · Forks 11.5k. · Issues: https://github.com/TryGhost/Ghost/issues · Blog: https://ghost.org/resources/
...
- **#26607** — Editor opening staff settings/profile triggers forbidden API calls and unrelated permission toast — needs:triage — Feb 26, 2026
...
Pinned mega-issues (unchanged): 🔥 Breaking Changes for 6.0 (#23924) · 🌐 i18n mega-issue (#23361) · Dependency Dashboard (#13265).
...
- **#9789** — Should Keystone enforce GraphQL query depth limits by default? — Feature — Mar 18, 2026 `[Inference: DoS/query-cost hardening]`
- **#9785** — `statelessSessions` attempts to use unsupported `Authorization: Basic` header rather than cookie — discussion / documentation / help wanted — Mar 6, 2026
...
- **#9766** — Document Fields Demo page refers to form that doesn't exist — Jan 24, 2026
...
Pinned: Node 20 (LTS) support (#8987, Jan 2024).

**Blog (carried forward, `[Unverified this cycle]`):** "A year of releases in review" — Aug 7 2024 (blog appears stale).
...
- **#7068** — Split `tinacms build` (pure codegen) from a deploy-time publish gate — @tinacms/cli / dx / enhancement — Jun 18, 2026
- **#7067** — Deploy schema gate should wait for schemaSha convergence, not a one-shot compare — @tinacms/cli / dx / enhancement — Jun 18, 2026

**Blog (carried forward, `[Unverified this cycle]`):** "Separate Content Repos are here for TinaCloud" — Jun 12.
...
**Newest / notable open issues (verified this run):**
- **#7875** — Path traversal in decap-server proxy allows read/write/delete of files outside the configured repository root — type: bug — Jul 5, 2026 `[Inference: trust-boundary vuln; vendor labeled bug, no CVE named]`
...
- **#7800** — Preview pane stops accepting scroll events after resizing the form/preview divider — ui/editor/preview-pane / type: bug — May 2, 2026

**Blog (carried forward, `[Unverified this cycle]`):** "Announcing Decap Turbo" — May 5.
...
**Newest / notable open issues (verified this run):**
- **#4501** — Security: Cross-origin code execution via unvalidated postMessage in builder-block (CWE-346) — Apr 4, 2026 `[vendor title names CWE-346; no CVE named]`
...
**Blog (carried forward, `[Unverified this cycle]`):** "How KPMG Closed the Design-to-Engineering Gap" — Jul 6.
...
**Newest / notable open issues (verified this run):**
- **#15406** — Local development setup for contributors is confusing, undocumented, and lacks hot reload for plugins — type: docs — May 14, 2026
- **#15399** — `/store/products` returns 500 for any `category_id` / `tag_id` filter when `index_engine` feature flag enabled — needs triaging / v2.0 — May 14, 2026
...
- **#15360** — Race condition in cart promotions can create duplicate line item adjustments — requires-team / bug / v2.0 — May 11, 2026 `[Inference: data-integrity]`
...
- **#15306** — Refund workflow can report success after partial refund failures (silent failure in `refundPaymentsStep`) — needs triaging / bug — May 6, 2026 `[Inference: financial-correctness]`
- **#15300** — `medusa db:migrate` exit code — bug / v2.0 — May 5, 2026
...
## Methodology & caveats
- **GitHub issue data** (open counts, newest issue IDs/titles/labels/dates, star/fork counts) was fetched and verified live this cycle for all 10 repositories via each repo's public Issues page.
- **Blog posts** could not be re-verified this cycle: the vendor blog pages are client-rendered and return page shells without executing JavaScript, so blog lines are carried forward from the last verified run and explicitly marked `[Unverified this cycle]`.
- **Security characterizations** rely on vendor-applied labels/titles. Where a severity or exploitability claim goes beyond what the vendor stated, it is marked `[Inference]`. CVE identifiers are only listed where the vendor published them.
- No write actions were taken against any external system (report-only, per scheduled-task policy).
