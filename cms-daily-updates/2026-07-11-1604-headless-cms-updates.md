...
**Date:** 2026-07-11 · **Run:** automated (scheduled task, 16:04 +07) · **Coverage:** 10 headless CMS projects
**Previous run:** 2026-07-11 15:04 (+07)
...
## TL;DR — Changes since previous run (2026-07-11 15:04)

**No change this cycle.** All 10 GitHub issue trackers returned the identical newest open issue (same ID, title, open date) and identical open-issue counts as the 15:04 run. Verified live this run.
...
- **#27094** — `@directus/api` uses an old version of tsdown and `openid-client` — Apr 11, 2026 — `[Inference]` security-relevant (outdated auth dependency)
- **#27091** — Save-as-copy throws error — Bug / Assets/Files / High Impact / Regression / Studio — Apr 10, 2026
- **#27042** — WYSIWYG not rendering when returning from edit then revisiting record as non-admin user (v11.16.1) — Bug / High Impact — Apr 3, 2026
...
- **#12794** — Presentation tool writes `sanity.previewUrlSecret` to wrong dataset in multi-workspace hosted Studio — SAPP — Apr 16, 2026 — `[Inference]` preview-secret isolation issue
...
- **#27445** — Security: add optional malware scanning for uploaded files (pompelmi) — needs:triage — Apr 17, 2026 (feature request, security-adjacent)
...
- **#9789** — Should Keystone enforce GraphQL query depth limits by default? — Feature — Mar 18, 2026 — `[Inference]` DoS-hardening request
...
- **#7134** — Reference fields fully hydrate on every keystroke during visual editing, silently exceeding the 1MB preview-overlay cap with no error — Jul 1, 2026
...
**Newest / notable open issues (verified this run):**
- **#7875** — Path traversal in `decap-server` proxy allows read/write/delete of files outside configured repo root — **type: bug (critical, trust-boundary)** — Jul 5, 2026
...
- **#15360** — [Bug] Race condition in cart promotions can create duplicate line item adjustments — requires-team / bug / v2.0 — May 11, 2026 — `[Inference]` data-integrity risk
- **#15306** — Refund workflow can report success after partial refund failures (silent failure in `refundPaymentsStep`) — needs triaging / bug — May 6, 2026 — `[Inference]` financial-integrity risk
...
## Notes & method
- **Verified this run:** all 10 GitHub issue trackers (open-issue counts + newest open issues, quoted from the live Issues pages).
- **Not fetched this run:** vendor blogs (client-rendered; carried forward from prior runs, labeled `[Unverified]`).
- **Labeling:** `[Inference]` = security/impact read derived from an issue's title/labels, not a confirmed advisory/CVE. `[Unverified]` = not confirmed on a fetched page this cycle.
- **Autonomous run:** scheduled task; no user present. No write actions taken against any external system — report only.
