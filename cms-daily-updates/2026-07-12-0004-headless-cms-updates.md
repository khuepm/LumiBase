...
**Date:** 2026-07-12 · **Run:** automated (scheduled task, 00:04 +07) · **Coverage:** 10 headless CMS projects
**Previous run:** 2026-07-11 23:04 (+07)

> Sources this run: each project's **GitHub Issues** page (open-issue counts + newest open issues with ID/title/labels/open-date), fetched live and verified this cycle for all 10. **Blogs** were not re-hydrated this cycle (client-rendered pages return shells without JS); "latest blog post" lines are **carried forward** from the last verified run (2026-07-11 23:04) and marked `[Unverified this cycle]`. Version numbers not printed on a fetched page are `[Unverified]`. Security-severity characterizations beyond a label/title the vendor itself applied are `[Inference]` (based on title/topic, not a confirmed advisory/CVE unless a CVE is named).

---

## TL;DR — Changes since previous run (2026-07-11 23:04)

**GitHub issues: no change.** All 10 trackers returned the identical newest open issue (same ID, title, open date) and identical open-issue counts as the 23:04 run. Verified live this run. This is the first run of 2026-07-12; the last new issue observed across all trackers remains **TinaCMS #7169** (Jul 7).

**Blogs: not re-verified this cycle** — carried forward from 23:04. Most recent activity on record: **Directus** — "AI is straining vulnerability disclosure for maintainers" (Jul 10) · **Builder.io** — "How KPMG Closed the Design-to-Engineering Gap" (Jul 6) · **Medusa** — "Announcing new Layout Composer in Medusa Admin" (Jul 1).
...
**Blog (carried forward, last verified 07-11 23:04):**
...
## Notes & method
- GitHub Issues fetched live this run for all 10 repos; counts and newest-open-issue IDs verified against the 2026-07-11 23:04 run — **no deltas**.
- Blog lines carried forward from the last verified run (client-rendered vendor blogs return shells to a non-JS fetch); flagged `[Unverified this cycle]`. Re-hydration can be forced with a JS-rendering browser fetch if a specific blog delta needs confirmation.
