...
**Date:** 2026-07-11 · **Run:** automated (scheduled task, 10:04 +07) · **Coverage:** 10 headless CMS projects
**Previous run:** 2026-07-11 09:04 (+07)

> Sources this run: each project's **GitHub Issues** page (open-issue counts + newest open issue ID/title/labels/open-date, quoted exactly as shown), fetched live and verified this cycle. Three vendor blogs (Strapi, Medusa, Directus) were re-fetched live; five (Payload, Sanity, Ghost, KeystoneJS, TinaCMS, Decap, Builder.io) were not fetched this run and rely on carried-forward context labeled `[Unverified]`. Version numbers not printed on a fetched page are `[Unverified]`. Security reads beyond issues explicitly labeled/titled as security are `[Inference]` (based on title/topic, not a confirmed advisory/CVE).

---

## TL;DR — Changes since previous run (2026-07-11 09:04)

**Tracker data: no change.** All 10 GitHub issue trackers returned the identical newest open issue (same ID, title, open date) and identical open-issue counts as the 09:04 run. Verified live this run.

**One blog change:** The **Directus** blog re-fetch this cycle returned a fully-hydrated article list, and the **Jul 10, 2026** post *"AI is straining vulnerability disclosure for maintainers"* (Rijk van Zanten) is **re-confirmed** as the latest article. The 09:04 run had downgraded this post to `[Unverified]` after a JS-hydrated fetch; it is now upgraded back to **verified**. Directus latest blog = Jul 10 (Product post Jun 29 "moving to a hardened Docker image" is next).

**Blogs re-fetched this cycle (unchanged latest):**
- **Strapi** — *"The Strapi MCP server is now GA"* (Jun 29, 2026) still top. CVE-disclosure post (CVE-2025-64526, CVE-2026-22599/22706/22707/27886) dated May 13, 2026 still live.
- **Medusa** — *"Announcing new Layout Composer in Medusa Admin"* (Jul 1, 2026) still top.
...
| Directus | 326 | #27129 (Apr 15) | AI is straining vuln disclosure (Jul 10) ✅ re-confirmed | #27094 outdated openid-client `[Inference]` |
...
**Blog highlights (live this run):** *"The Strapi MCP server is now GA"* (Jun 29) is the top post. Also live: June Community Call recap (Jun 29), release roundup Mar–Jun 2026 (Jun 18), "How To Extend Strapi's MCP Server With Custom Tools via a Plugin" (Jun 13), and the CVE-disclosure post (May 13).
...
**Blog highlights (live this run — re-confirmed):** *"AI is straining vulnerability disclosure for maintainers"* by Rijk van Zanten (**Jul 10, 2026**) is the latest article. Also live: *"We're moving to a hardened Docker image. Here's what that means."* (Jun 29), *"Headless CMS with AI Capabilities: What to Look For"* (Jun 26), *"We've Issued 1,000+ Free License Keys"* (Jun 22), *"Best Headless CMS in 2026"* (Jun 16), and the v12 "A Backend for Everyone on Your Team" product post (May 26, adds native draft/publish workflows, redesigned Studio, AI-assisted translations, JSON filtering, OAuth 2.1 for MCP).
...
**Blog highlights (live this run):** *"Announcing new Layout Composer in Medusa Admin"* by Nicolas Gorga (Jul 1) is the top post.
...
## Methodology & caveats
- GitHub open-issue counts and newest-issue details are quoted exactly from the live issue pages at fetch time (~10:04 +07). Only three vendor blogs (Strapi, Medusa, Directus) were re-fetched live this run; the other blogs are labeled `[Unverified]` and rely on carried-forward context.
...
- No tracker deltas vs. the 09:04 run. The only change this cycle: the Directus **Jul 10** blog post *"AI is straining vulnerability disclosure for maintainers"* was re-confirmed via a clean fetch and upgraded from `[Unverified]` (as recorded at 09:04) back to **verified**.
