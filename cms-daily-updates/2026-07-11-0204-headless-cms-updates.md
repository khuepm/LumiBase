...
**Date:** 2026-07-11 · **Run:** automated (scheduled task, 02:04 +07) · **Coverage:** 10 headless CMS projects
**Previous run:** 2026-07-11 01:05 (+07)

> Sources this run: each project's **GitHub Issues** page, fetched live and verified (open-issue counts, newest open issue ID/title/open-date quoted exactly as shown). **Blog/release highlights are carried forward from the 01:05 run and were not re-fetched this run** — they are labeled `[carried forward]` below. Specific version numbers not printed on a fetched page are labeled `[Unverified]`.

---

## TL;DR — Changes since previous run (2026-07-11 01:05)

**Tracker data: no change.** All 10 GitHub issue trackers returned the identical newest open issue (same ID, title, open date) and identical open-issue counts as the 01:05 run. Verified live this run.
...
> **Note on cadence:** trackers were identical to a run one hour earlier, so blogs were not re-crawled this cycle (they change at most daily). Blog highlights below are `[carried forward]` from 01:05.
...
**Content Manager stability cluster (Priority: Urgent / critical):** #26434, #26396, #26389 (undefined-property crashes navigating single types / admin UI).
...
**Blog highlights `[carried forward]`:** "An early look at Payload 4.0: Admin UI Redesign, TanStack, MCP, and More" (Jun 9); "Critical Security Notice Affecting React 19 and Next.js" (Dec 4, 2025); Payload is now part of Figma (Jun 2025).
...
- **#12794** — Presentation tool writes `previewUrlSecret` to wrong dataset in multi-workspace hosted Studio — SAPP — Apr 16, 2026
...
**Note:** blog is low-frequency (newest "A year of releases in review", Aug 2024); product news now lands mainly in GitHub Releases.
...
**Signal:** active v4 track work (auth plugin, CLI build/deploy split, rich-text improvements). Repo now shows "issue creation is restricted."
...
- **#7816 / #7802 / #7800** — Soft line breaks / copy-paste / scroll issues in new richtext (Plate) widget — type: bug — May 2026
...
- **`[carried forward]`:** blog/release highlights were not re-crawled this run (trackers were identical to a run one hour prior; blogs change at most daily). They reflect the 01:05 crawl.
...
*Generated automatically — headless CMS watch, run 2026-07-11 02:04 (+07).*
