...
**Date:** 2026-07-11 · **Run:** automated (scheduled task, 09:04 +07) · **Coverage:** 10 headless CMS projects
**Previous run:** 2026-07-11 08:03 (+07)

> Sources this run: each project's **GitHub Issues** page (open-issue counts + newest open issue ID/title/labels/open-date quoted exactly as shown), fetched live and verified this cycle. Three vendor blogs (Strapi, Directus, Medusa) were re-fetched live; five (Ghost, KeystoneJS, TinaCMS, Decap, Builder.io) were not fetched this run and rely on carried-forward context labeled `[Unverified]`. Version numbers not printed on a fetched page are labeled `[Unverified]`. Security reads beyond explicitly-labeled issues are marked `[Inference]`.

---

## TL;DR — Changes since previous run (2026-07-11 08:03)

**Tracker data: no change.** All 10 GitHub issue trackers returned the identical newest open issue (same ID, title, open date) and identical open-issue counts as the 08:03 run. Verified live this run.

**Blogs re-fetched this cycle:**
- **Strapi blog — unchanged latest:** *"The Strapi MCP server is now GA"* — **Jun 29, 2026** (still the top post). CVE-disclosure post (CVE-2025-64526, CVE-2026-22599/22706/22707/27886) dated May 13, 2026 also live.
- **Medusa blog — unchanged latest:** *"Announcing new Layout Composer in Medusa Admin"* — **Jul 1, 2026**.
- **Directus blog — `[Unverified]` this run:** the live fetch returned a JS-hydrated/rotating article list whose "All Articles" top entry was *"Evolving Our License for Long-Term Sustainability"* (Apr 22, 2026). The Jul 10 post *"AI is straining vulnerability disclosure for maintainers"* seen in the previous run was **not** re-confirmed in this fetch; treat prior-run latest as unverified this cycle.
...
| Directus | 326 | #27129 (Apr 15) | v12 license change (Apr 22) `[Unverified]` | #27094 outdated openid-client `[Inference]` |
| Payload | 288 | #16288 (Apr 15) | Early look at Payload 4.0 (Jun 9) `[Unverified]` | — |
| Sanity | 75 | #12870 (May 24) | not fetched this run `[Unverified]` | — |
...
**Cross-cutting signals:** AI/agent tooling continues to dominate the fetched blogs (Strapi MCP GA, Medusa MCP/agent tooling, Directus native MCP). Framework-migration friction persists as a bug driver — Next.js 16.2 / Turbopack on Payload (#16286, #16288), Node 22/24 on TinaCMS (#7162, #7109) and Builder.io (#4137), React 19 peer-dep conflicts on Medusa (#15398). Two critical, still-open trust-boundary vulns (Decap path traversal #7875, Builder postMessage RCE #4501) remain unresolved.
...
**Blog highlights (live this run):** *"The Strapi MCP server is now GA"* (Jun 29) is the top post. Also live: June Community Call recap (Jun 29), release roundup Mar–Jun 2026 (Jun 18), and the CVE-disclosure post (May 13).
...
**Blog highlights:** `[Unverified]` this run — the live fetch returned a rotating/JS-hydrated list; the "All Articles" top entry was *"Evolving Our License for Long-Term Sustainability"* (Apr 22, 2026). The Jul 10 post *"AI is straining vulnerability disclosure for maintainers"* from the prior run was not re-confirmed this cycle.
...
**Blog highlights (live this run):** *"Announcing new Layout Composer in Medusa Admin"* by Nicolas Gorga (Jul 1).
...
## Methodology & caveats
- GitHub open-issue counts and newest-issue details are quoted exactly from the live issue pages at fetch time (~09:04 +07). Only three vendor blogs (Strapi, Directus, Medusa) were re-fetched live this run; the other five are labeled `[Unverified]` and rely on carried-forward context.
...
- No tracker deltas vs. the 08:03 run. The only notable change this cycle is that the Directus blog's Jul 10 post (seen previously) was **not** re-confirmed in this fetch and is therefore downgraded to `[Unverified]`.
