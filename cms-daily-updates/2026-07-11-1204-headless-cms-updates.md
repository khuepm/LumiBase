...
**Date:** 2026-07-11 · **Run:** automated (scheduled task, 12:04 +07) · **Coverage:** 10 headless CMS projects
**Previous run:** 2026-07-11 10:04 (+07)

> Sources this run: each project's **GitHub Issues** page (open-issue counts + newest open issue ID/title/labels/open-date, quoted exactly as shown), fetched live and verified this cycle for all 10. Three vendor blogs (Strapi, Medusa, Directus) were re-fetched live this run; the other five (Payload, Sanity, Ghost, KeystoneJS, TinaCMS, Decap, Builder.io) were not fetched this run and rely on carried-forward context labeled `[Unverified]`. Version numbers not printed on a fetched page are `[Unverified]`. Security reads beyond issues explicitly labeled/titled as security are `[Inference]` (based on title/topic, not a confirmed advisory/CVE).

---

## TL;DR — Changes since previous run (2026-07-11 10:04)

**No change this cycle.** All 10 GitHub issue trackers returned the identical newest open issue (same ID, title, open date) and identical open-issue counts as the 10:04 run. Verified live this run.

**Blogs re-fetched this cycle (all unchanged latest):**
...
- **Directus** — *"AI is straining vulnerability disclosure for maintainers"* (Rijk van Zanten, Jul 10, 2026) still the latest article; fully-hydrated fetch, re-confirmed **verified** (Product post Jun 29 "moving to a hardened Docker image" is next).
...
| Directus | 326 | #27129 (Apr 15) | AI is straining vuln disclosure (Jul 10) ✅ verified | #27094 outdated openid-client `[Inference]` |
...
## Methodology & caveats
- GitHub open-issue counts and newest-issue details are quoted exactly from the live issue pages at fetch time (~12:04 +07), verified for all 10 projects. Three vendor blogs (Strapi, Medusa, Directus) were re-fetched live this run; the other blogs are labeled `[Unverified]` and rely on carried-forward context.
...
- No tracker deltas and no blog deltas vs. the 10:04 run. All three re-fetched blogs (Strapi, Medusa, Directus) returned the same latest posts. This was a fully steady-state cycle.
