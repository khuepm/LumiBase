...
--      SELECT lower(email), count(*) FROM lumibase_users GROUP BY 1 HAVING count(*) > 1;
--    and merge/remove the extras before running this migration.

ALTER TABLE "lumibase_users" ADD COLUMN IF NOT EXISTS "password_changed_at" timestamp;--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS "users_email_lower_unique" ON "lumibase_users" USING btree (lower("email"));
