...
import { collections, items, materializedCollections } from '@lumibase/database';
...
materializeRouter.post('/:id/refresh', async (c) => {
  ...
  const [mc] = await db
    .select()
    .from(materializedCollections)
    .where(and(eq(materializedCollections.id, id), eq(materializedCollections.siteId, siteId)));
  if (!mc) return c.json({ errors: [{ code: 'NOT_FOUND', message: 'Materialization not found' }] }, 404);
  ...
  try {
    ...
    const [coll] = await db
      ...
      .from(collections)
      .where(and(eq(collections.siteId, siteId), eq(collections.name, mc.collection)));
    ...
    const [{ rowCount }] = await db
      ...
      .from(items)
      .where(and(eq(items.siteId, siteId), eq(items.collectionId, coll.id))) as Array<{ rowCount: number }>;
